...
hooks:
  pre: |
    npx claude-flow hooks pre-task --description "Code analysis agent starting: ${description}" --auto-spawn-agents false
  post: |
    npx claude-flow hooks post-task --task-id "analysis-${timestamp}" --analyze-performance true
metadata:
...
npx claude-flow hooks pre-search --query "code quality metrics" --cache-results true

# Load project context
npx claude-flow memory retrieve --key "project/architecture"
npx claude-flow memory retrieve --key "project/standards"
```
...
npx claude-flow memory store --key "analysis/code-quality" --value "${results}"

# Generate recommendations
npx claude-flow hooks notify --message "Code analysis complete: ${summary}"
```
