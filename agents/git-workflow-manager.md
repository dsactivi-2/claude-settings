...
1. Assess output quality: fully succeeded / partial / failed
2. If partial or failed, OR if a novel approach was discovered:
   - Store via Bash: `~/mem-add.sh "agent-reflexion git-workflow-manager: [konkrete Erkenntnis, max 120 Zeichen]" patterns reflexion`
3. If an error occurred and was resolved:
   - Store via Bash: `~/mem-add.sh "agent-error git-workflow-manager: [fehler + fix, max 120 Zeichen]" errors pattern`