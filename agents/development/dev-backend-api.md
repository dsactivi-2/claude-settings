...
hooks:
  pre_execution: |
    ...
    # Pattern search via MCP: mcp__claude-flow__memory_search
    # Query: "API implementation: $TASK" with k=5, min-reward=0.85
    # Pattern stats via MCP: mcp__claude-flow__memory_search
    # Query: "API implementation" with k=5

    # Store task start for learning
    # Pattern store via MCP: mcp__claude-flow__memory_store
    # Key: "backend-dev-$(date +%s)", value: "API: $TASK | status: started"
  ...
  post_execution: |
    ...
    # Pattern store via MCP: mcp__claude-flow__memory_store
    # Key: "backend-dev-$(date +%s)", value: "API: $TASK | reward: $REWARD | success: $SUCCESS"
    # Critique: "API implementation with N endpoints"

    # Train neural patterns on successful implementations
    if [ "$SUCCESS" = "true" ]; then
      echo "🧠 Training neural pattern from successful API implementation"
      # Neural train via MCP: mcp__claude-flow__neural_train
      # Pattern type: "coordination", training data: "$TASK_OUTPUT", epochs: 50
    fi
  ...
  on_error: |
    ...
    # Pattern store via MCP: mcp__claude-flow__memory_store
    # Key: "backend-dev-$(date +%s)", value: "API: $TASK | Failed: {{error_message}} | reward: 0.0 | success: false"
examples:
