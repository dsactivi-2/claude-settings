...
2. If partial or failed, OR if a novel approach was discovered:
   - Store via Bash: `~/mem-add.sh "agent-reflexion mcp-developer: [konkrete Erkenntnis, max 120 Zeichen]" patterns reflexion`
3. If an error occurred and was resolved:
   - Store via Bash: `~/mem-add.sh "agent-error mcp-developer: [fehler + fix, max 120 Zeichen]" errors pattern`